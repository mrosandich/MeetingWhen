...
SlickDTP.prototype.pickDate = function(MySQLDateId,PrettyId)
{
	...
	{
		var mainParts = $(this.current_id).val();
		var mainParts2 = $(this.pretty_id).val();
		
		if(mainParts.indexOf("/")>0)
		{
			var dateparts = mainParts.split("/");
			this.selected_year = dateparts[2];
			this.selected_month = dateparts[0];
			this.selected_day = dateparts[1];
			
			$('#datepicker').datepicker("setDate", new Date(this.selected_year,this.selected_month-1,this.selected_day) );
		}
		
		if(mainParts2.indexOf(":")>0)
		{
			if( mainParts2.indexOf("am") >0 ){
				this.selected_ampm = "AM";
				mainParts2 = mainParts2.replace("am","");
			}
			if( mainParts2.indexOf("pm") >0 ){
				this.selected_ampm = "PM";
				mainParts2 = mainParts2.replace("pm","");
			}
			var timeparts = mainParts2.split(":");
			this.selected_hour = timeparts[0];
			this.selected_min = timeparts[1];
			this.setMinCircle(this.selected_min);
			if(this.selected_ampm == "PM" ){
				$('.am_slide').hide();
				$('.pm_slide').show();
				$('.am_slide_l').show();
			}
...
SlickDTP.prototype.dateDone = function(){
	$(this.current_id).val(this.selected_month + '/' + this.selected_day + '/' + this.selected_year );
	$(this.pretty_id).val(this.selected_hour + ':' + this.selected_min + this.selected_ampm.toLowerCase());
